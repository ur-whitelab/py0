...
class ContactInfectionLayer(tf.keras.layers.Layer):
    ...
    def call(self, neff, ntot):
        fxn = contact_infection_func(self.infectious_compartments)
        p = fxn(neff, ntot, self.beta)
        return p


def contact_infection_func(infectious_compartments, area=None):
    if area is not None:
        def density_fxn(n, area=area, xi=0.01):
